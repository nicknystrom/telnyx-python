...
class TelnyxObject(dict):
    ...
    def __init__(self, id=None, api_key=None, last_response=None, **params):
        super(TelnyxObject, self).__init__()
    ...
    def update(self, update_dict):
        ...
        return super(TelnyxObject, self).update(update_dict)

    def __setattr__(self, k, v):
        if k[0] == "_" or k in self.__dict__:
            return super(TelnyxObject, self).__setattr__(k, v)
    ...
    def __delattr__(self, k):
        if k[0] == "_" or k in self.__dict__:
            return super(TelnyxObject, self).__delattr__(k)
        else:
            del self[k]
    ...
    def __setitem__(self, k, v):
        ...
        super(TelnyxObject, self).__setitem__(k, v)

    def __getitem__(self, k):
        try:
            return super(TelnyxObject, self).__getitem__(k)
        except KeyError as err:
            if k in self._transient_values:
                raise KeyError(
                    "%r.  HINT: The %r attribute was set in the past."
                    "It was then wiped when refreshing the object with "
                    "the result returned by Telnyx's API, probably as a "
    ...
    def __delitem__(self, k):
        super(TelnyxObject, self).__delitem__(k)
    ...
    def construct_from(cls, values, key, last_response=None):
        instance = cls(values.get("id"), api_key=key, last_response=last_response)
        instance.refresh_from(values, api_key=key, last_response=last_response)
        return instance
    ...
    def refresh_from(self, values, api_key=None, partial=False, last_response=None):
        ...
        for k, v in six.iteritems(values):
            super(TelnyxObject, self).__setitem__(
                k, util.convert_to_telnyx_object(v, api_key)
            )
    ...
    def __repr__(self):
        ...
        if isinstance(self.get("record_type"), six.string_types):
            ident_parts.append(self.get("record_type"))
    ...
    def __str__(self):
        return json.dumps(
            self.to_dict_recursive(), sort_keys=True, indent=2, cls=self.ReprJSONEncoder
        )
    ...
    def __copy__(self):
        copied = TelnyxObject(self.get("id"), self.api_key)
        ...
        for k, v in six.iteritems(self):
            ...
            super(TelnyxObject, copied).__setitem__(k, v)
    ...
    def __deepcopy__(self, memo):
        ...
        for k, v in six.iteritems(self):
            # Call parent's __setitem__ to avoid checks that we've added in the
            # overridden version that can throw exceptions.
            super(TelnyxObject, copied).__setitem__(k, deepcopy(v, memo))
