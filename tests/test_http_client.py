...
class TelnyxClientTestCase(object):
    ...
    def request_mocks(self, mocker):
        ...
        for lib in self.REQUEST_LIBRARIES:
            request_mocks[lib] = mocker.patch("telnyx.http_client.%s" % (lib,))
        return request_mocks


class TestNewDefaultHttpClient(TelnyxClientTestCase):
    @pytest.fixture(autouse=True)
    def setup_warnings(self, request_mocks):
        original_filters = telnyx.http_client.warnings.filters[:]
        telnyx.http_client.warnings.simplefilter("ignore")
        yield
        telnyx.http_client.warnings.filters = original_filters

    def check_default(self, none_libs, expected):
        for lib in none_libs:
            setattr(telnyx.http_client, lib, None)

        inst = telnyx.http_client.new_default_http_client()

        assert isinstance(inst, expected)

    def test_new_default_http_client_urlfetch(self):
        self.check_default((), telnyx.http_client.UrlFetchClient)

    def test_new_default_http_client_requests(self):
        self.check_default(("urlfetch",), telnyx.http_client.RequestsClient)

    def test_new_default_http_client_pycurl(self):
        self.check_default(("urlfetch", "requests"), telnyx.http_client.PycurlClient)

    def test_new_default_http_client_urllib2(self):
        self.check_default(
            ("urlfetch", "requests", "pycurl"), telnyx.http_client.Urllib2Client
        )


class TestRetrySleepTimeDefaultHttpClient(TelnyxClientTestCase):
    ...
    def mock_max_delay(self, new_value):
        original_value = telnyx.http_client.HTTPClient.MAX_DELAY
        telnyx.http_client.HTTPClient.MAX_DELAY = new_value
        try:
            yield self
        finally:
            telnyx.http_client.HTTPClient.MAX_DELAY = original_value

    def test_sleep_time_exponential_back_off(self):
        client = telnyx.http_client.new_default_http_client()
    ...
    def test_initial_delay_as_minimum(self):
        ...
        initial_delay = telnyx.http_client.HTTPClient.INITIAL_DELAY
        self.assert_sleep_times(client, [initial_delay] * 5)

    def test_maximum_delay(self):
        client = telnyx.http_client.new_default_http_client()
        client._add_jitter_time = lambda t: t
        max_delay = telnyx.http_client.HTTPClient.MAX_DELAY
    ...
    def test_randomness_added(self):
        ...
        base_value = telnyx.http_client.HTTPClient.INITIAL_DELAY * random_value

        with self.mock_max_delay(10):
            expected = [
                telnyx.http_client.HTTPClient.INITIAL_DELAY,
                base_value * 2,
                base_value * 4,
                base_value * 8,
                base_value * 16,
    ...
    def test_jitter_has_randomness_but_within_range(self):
        ...
        jittered_ones = set(map(lambda _: client._add_jitter_time(1), list(range(100))))

        assert len(jittered_ones) > 1
        assert all(0.5 <= val <= 1 for val in jittered_ones)


class TestRetryConditionsDefaultHttpClient(TelnyxClientTestCase):
    ...
    def test_should_retry_on_num_retries(self, mocker):
        ...
        assert (
            client._should_retry(None, api_connection_error, max_test_retries + 1)
            is False
        )
        assert (
            client._should_retry((None, 409, None), None, max_test_retries + 1) is False
        )
...
class ClientTestBase(object):
    ...
    def valid_url(self, path="/foo"):
        return "https://api.telnyx.com%s" % (path,)
...
class TestRequestsClient(TelnyxClientTestCase, ClientTestBase):
    REQUEST_CLIENT = telnyx.http_client.RequestsClient
...
class TestRequestClientRetryBehavior(TestRequestsClient):
    ...
    def mock_retry(self, mocker, session, request_mock):
        def mock_retry(retry_error_num=0, no_retry_error_num=0, responses=[]):
            ...
            session.request.side_effect = retry_errors + no_retry_errors + responses
    ...
    def test_retry_error_until_response(self, mock_retry, response, check_call_numbers):
        mock_retry(retry_error_num=1, responses=[response(code=202)])
        _, code, _ = self.make_request()
        assert code == 202
        check_call_numbers(2)

    def test_retry_error_until_exceeded(self, mock_retry, response, check_call_numbers):
    ...
    def test_retry_codes_until_exceeded(self, mock_retry, response, check_call_numbers):
    ...
    def connection_error(self, session):
        ...
        def connection_error(given_exception):
            with pytest.raises(telnyx.error.APIConnectionError) as error:
                client._handle_request_error(given_exception)
            return error.value

        return connection_error

    def test_handle_request_error_should_retry(self, connection_error, mock_retry):
    ...
    def test_handle_request_error_should_not_retry(self, connection_error, mock_retry):
        request_mock = mock_retry()

        error = connection_error(request_mock.exceptions.SSLError())
        assert error.should_retry is False
        assert "not verify Telnyx's SSL certificate" in error.user_message
...
class TestUrlFetchClient(TelnyxClientTestCase, ClientTestBase):
    REQUEST_CLIENT = telnyx.http_client.UrlFetchClient
...
class TestUrllib2Client(TelnyxClientTestCase, ClientTestBase):
    REQUEST_CLIENT = telnyx.http_client.Urllib2Client
    ...
    def make_request(self, method, url, headers, post_data, proxy=None):
        ...
        self.proxy = proxy
        return self.client.request_with_retries(method, url, headers, post_data)
...
class TestPycurlClient(TelnyxClientTestCase, ClientTestBase):
    REQUEST_CLIENT = telnyx.http_client.PycurlClient
    ...
    def bio_getvalue(self, mocker):
        bio_patcher = mocker.patch("telnyx.util.io.BytesIO")
    ...
    def mock_error(self):
        def mock_error(mock):
            ...
            telnyx.http_client.pycurl.error = FakeException
            mock.perform.side_effect = telnyx.http_client.pycurl.error
    ...
    def check_call(self, request_mocks):
        def check_call(mock, method, url, post_data, headers):
            ...
            mock.setopt.assert_any_call(lib_mock.URL, telnyx.util.utf8(url))
            ...
            if method == "get":
                ...
                mock.setopt.assert_any_call(lib_mock.CUSTOMREQUEST, method.upper())
...
class TestAPIEncode(TelnyxClientTestCase):
    def test_encode_dict(self):
        body = {"foo": {"dob": {"month": 1}, "name": "bat"}}

        values = [t for t in telnyx.api_requestor._api_encode(body)]
